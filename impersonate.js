...
async function getSaToken() {
    ...
    const resp = await fetch('https://sso.csh.rit.edu/auth/realms/master/protocol/openid-connect/token', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'Authorization': `Basic ${Buffer.from(process.env.GK_SA_USERNAME + ":" + process.env.GK_SA_PASSWORD).toString('base64')}`,
        },
...
async function getUidFromUsername(username, saToken) {
    ...
    const resp = await fetch('https://sso.csh.rit.edu/auth/admin/realms/csh/users?username=' + username, {
        headers: {
...
async function getImpersonationSession(userId, saToken) {
    ...
    const resp = await fetch(`https://sso.csh.rit.edu/auth/admin/realms/csh/users/${userId}/impersonation`, {
        method: 'POST',
        headers: {
            'Authorization': `Bearer ${saToken}`,
        }
    ...
    const identity = identityRegex.exec(cookies)[0].split('=')[1].replace(';', '');
...
async function getUserToken(identity, session) {
    ...
    const resp = await fetch('https://sso.csh.rit.edu/auth/realms/csh/protocol/openid-connect/auth?client_id=gatekeeper&response_type=token&response_mode=fragment&redirect_uri=https%3A%2F%2Fgatekeeper.csh.rit.edu%2Fcallback', {
        method: 'GET',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
